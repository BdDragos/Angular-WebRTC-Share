...
@Component({
  selector: 'app-video-call-component',
  templateUrl: './video-call-component.component.html',
  styleUrls: ['./video-call-component.component.scss'],
})
export class VideoCallComponentComponent implements OnInit {
  public title = 'webrtc';
  ...
  @Output('callback') callback: EventEmitter<Object> = new EventEmitter<Object>();

  constructor(private socketIOService: CommunicationService, private router: Router, private changeDetector: ChangeDetectorRef) {}
  ...
  ngOnInit() {
    ...
    this.socketIOService.ReceiveCallRequest().subscribe((data) => {
  ...
  OnCallRequestReceived(data) {
    ...
    // console.log(data);
    if (data.desc) {
      let descrip = new RTCSessionDescription(data.desc);
  ...
  OnSetRemoteSuccess(val) {
    ...
    this.peerConnection
      .createAnswer()
      .then(this.OnCreateAnswerSuccess.bind(this), this.OnCreateSessionDescriptionError.bind(this));
  }
  ...
  OnCallAnswer(descrip) {
    ...
    this.peerConnection
      .setRemoteDescription(new RTCSessionDescription(descrip))
      .then(() => this.ShowSuccess('Succesfully set remote description'))
      .catch((err) => {
        console.log(err);
      });
  }
  ...
  GotLocalStream(stream) {
    let lv = document.getElementById('local-video') as HTMLVideoElement;
    lv.srcObject = stream;
    lv.controls = false;
    lv.muted = true;
    lv.volume = 0;
  ...
  OnIceCandidate(conn, event) {
    if (event.candidate) {
      ...
      let candi = new RTCIceCandidate(event.candidate);
  ...
  GotRemoteStream(stream) {
    ...
    let lv = document.getElementById('remote-video') as HTMLVideoElement;
    lv.srcObject = stream;
  ...
  EndCall() {
    let caller = this.socketIOService.connectedusers.find((a) => a.id == this.caller);
    this.socketIOService.EndVideoCall(this.loggedUserName, this.caller, caller.username);
    this.CallBack();
  }
  OnCallEnded() {
    this.socketIOService.OnVideoCallEnded().subscribe((data) => {
      if (data) {
